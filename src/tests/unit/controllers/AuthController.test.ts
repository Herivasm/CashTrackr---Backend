...
describe('AuthController.createAccount', () => {
    ...
    it('should return a 409 status and an error message if the email is already registered', async () => {
        ...
        expect(data).toHaveProperty('error', 'Este correo ya está en uso')
    ...
    it('should register a new user and return a success message', async () => {
        const req = createRequest({
            method: 'POST',
            url: 'api/auth/create-account',
            body: { email: "test@test.com", password: 'testpassword', name: 'test name' }
        })
        const res = createResponse();
...
describe('AuthController.login', () => {
    it('should return 404 if user is not found', async () => {
        ...
        expect(data).toEqual({ error: 'Este usuario no existe' })
    })
